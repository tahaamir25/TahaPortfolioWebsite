...
const gradientBorder: Record<ProjectColor, string> = {
  blue:    "bg-gradient-to-br from-accent/40 via-violet-500/20 to-transparent",
  violet:  "bg-gradient-to-br from-violet-500/40 via-accent/20 to-transparent",
  emerald: "bg-gradient-to-br from-emerald-500/40 via-accent/20 to-transparent",
};

const colorStyles: Record<
  ProjectColor,
  { badge: string; dot: string; link: string }
> = {
  blue: {
...
export default function ProjectCard({ project }: { project: Project }) {
  ...
  return (
    <div
      className={`p-[1px] rounded-xl ${gradientBorder[project.color]} shadow-sm dark:shadow-none hover:shadow-lg dark:hover:shadow-[0_0_28px_rgba(41,151,255,0.12)] hover:-translate-y-0.5 transition-all duration-300`}
      onClick={() => setFlipped((prev) => !prev)}
    >
      <div
        style={{ perspective: "1200px" }}
        className="min-h-[300px] cursor-pointer rounded-[11px] overflow-hidden"
      >
        <div
          style={{
        ...
        >
          ...
          <div
            ...
            className="absolute inset-0 flex flex-col bg-card rounded-[11px] p-5"
          >
